...
client.on("ready", async () => {
  ...
  // The Bots status
  client.user.setActivity("luna info • lunabot.ml", { type: "PLAYING" });
  ...
  //Bot Data About the Bot
  let bot = await Bot.findOne({ Id: client.user.id });
  if (!bot) {
      const newBot = await Bot.create({ Id: client.user.id })
      bot = newBot
  }
